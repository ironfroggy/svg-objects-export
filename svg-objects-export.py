...
def exportObject(id, name, args,prefix,extension,infile):
	debug("exporting ", id)
	destfile = ''.join([args.destdir, prefix, name, '.', extension])
	export = args.force
	if not args.force:
	...
	if export:
		message('  '+id+' to '+destfile)
		command = args.inkscape+' -i "'+id+'" --export-'+args.type+' "'+destfile+'" '+args.extra+' '+infile
	...
	if (regexp_mode):
		...
		for obj in objects_all.splitlines():
			...
			if ((args.exclude and (match == None)) or (not args.exclude and (match != None)) ):
				exportObject(obj, obj,args,prefix,extension,infile)
	elif (xpath_mode):
		...
		if (len(parser.error_log) > 0):
			...
			debug(error_log)
		find = etree.XPath("("+args.xpath+")", namespaces=xpath_namespaces) #find the ids, not the objects
		objects = dict(
			(obj.get('id'), obj)
			for obj in find(intree)
		)
		message("found %i objects matching XPath" % len(objects))
		if (not args.exclude): #include mode
			for id, obj in objects.items():
				name = obj.get('{http://www.inkscape.org/namespaces/inkscape}label')
				exportObject(id, name,args,prefix,extension,infile)
		else: #exclude mode
			objects_all = subprocess.check_output([args.inkscape, "--query-all", infile])
			#message(objects)
			for id in objects_all.splitlines():
				obj = objects[id]
				id = id.split(',')[0] #keep only ID:
				if not(id in objects):
					name = obj.get('{http://www.inkscape.org/namespaces/inkscape}label')
					exportObject(id,obj,args,prefix,extension,infile)
